...
class fqlRepayFile(EnvInit):
    ...
    def repayment_acct_period(self, temple):
        ...
        # 提前结清
            ...
            temple['paid_int_amt'] = str('{:.2f}'.format(0))

            pre_repay_date = str(asset_repay_plan["start_date"])
            self.log.info("资产时间:{}".format(pre_repay_date))
            self.log.info("huankuan时间:{}".format(self.repay_date))
            ...
            if pre_repay_date > repay_date:
                temple["paid_prin_amt"] = str('{:.2f}'.format(0))  # 如果还款时间小于账单开始时间，利息应该为0
            else:
