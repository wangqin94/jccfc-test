...
"""
@Project ：jccfc-test 
@File    ：XiaoXCheckBizImpl.py
@Author  ：jccfc
@Date    ：2022/07/06 15:55
"""
import time

from src.enums.EnumXiaoX import StatusCodeEnum, XiaoXApiCreditStatusEnum, XiaoXApiLoanStatusEnum, XiaoXApiRepayStatusEnum
...
class XiaoXCheckBizImpl(XiaoXBizImpl):
        ...
        for n in range(flag):
            ...
            try:
                if head['returnCode'] == StatusCodeEnum.SUCCESS.code and head['returnMessage'] == StatusCodeEnum.SUCCESS.msg:
                    ...
                    if status == XiaoXApiCreditStatusEnum.SUCCESS.value:
                        self.log.demsg('接口层查询：授信成功')
                        return status
                    elif status == XiaoXApiCreditStatusEnum.FAIL.value:
                        self.log.error('接口层授信失败,状态：{},失败原因:{}'.format(status, res['body']['rejectMsg']))
                        raise AssertionError('授信失败，接口层状态不符合预期')
                    elif status == XiaoXApiCreditStatusEnum.NOTEXIST.value:
                        self.log.error('查无此单')
                        raise AssertionError('授信失败，接口层状态不符合预期')
                    elif status == XiaoXApiCreditStatusEnum.DEALING.value:
    ...
    def XiaoX_check_loan_apply_status(self, thirdApplyId):
        """
        @param thirdApplyId:  授信申请单号
        @return:
        """
        ...
        for n in range(flag):
            ...
            try:
                if head['returnCode'] == StatusCodeEnum.SUCCESS.code and head['returnMessage'] == StatusCodeEnum.SUCCESS.msg:
                    ...
                    if status == XiaoXApiLoanStatusEnum.SUCCESS.value:
                        self.log.demsg('支用成功')
                        return status
                    elif status == XiaoXApiLoanStatusEnum.FAIL.value:
                        self.log.error('支用失败,状态：{},失败原因:{}'.format(status, res['body']['loanResultDesc']))
                        raise AssertionError('支用失败，接口层状态不符合预期')
                    elif status == XiaoXApiLoanStatusEnum.DEALING.value:
    ...
    def XiaoX_check_repay_status(self, repayApplySerialNo):
        """
        ...
        for n in range(flag):
            ...
            try:
                if returnCode == StatusCodeEnum.SUCCESS.code and returnMessage == StatusCodeEnum.SUCCESS.msg:
                    ...
                    if status == XiaoXApiRepayStatusEnum.REPAY_SUCCESS.value:
                        self.log.demsg('还款成功')
                        return status
                    elif status == XiaoXApiRepayStatusEnum.REPAY_FAIL.value:
                        self.log.error('还款失败,状态：{},失败原因:{}'.format(status, repayStatusDesc))
                        raise AssertionError('还款失败，接口层状态不符合预期')
                    elif status == XiaoXApiRepayStatusEnum.NO_BILL.value:
                        self.log.error('还款失败,状态：{},失败原因:{}'.format(status, repayStatusDesc))
                        raise AssertionError('还款失败，查无此单')
                    elif status == XiaoXApiRepayStatusEnum.REPAY_REPAYING.value:
...
if __name__ == '__main__':
    pass
    # XiaoXCheckBizImpl().XiaoX_check_credit_apply_status("creditApplyNo16435111528004788")
