...
class JuZiBizImpl(MysqlInit):
    def __init__(self, data=None,  encrypt_flag=True, person=True):
        ...
        self.interestRate = getInterestRate(ProductIdEnum.JUZI.value)
        # 初始化产品、商户
        self.productId = ProductIdEnum.JUZI.value
        self.merchantId = EnumMerchantId.JUZI.value
    ...
    def get_user_info(self, data=None, person=True):
        ...
        if data:
            ...
            if person:
                base_data = get_base_data(str(self.env) + ' -> ' + str(ProductEnum.JUZI.value))
            else:
    ...
    # 还款申请
    def repay_apply(self, loanInvoiceId, repay_scene='01', repay_type='1', repayTerm=None, repayGuaranteeFee=None,
        ...
        # repay_apply_data["repayInterest"] = 5.36
        ...
        if repay_type == "1":
            ...
            # repay_apply_data["repayAmount"] = 86.37
        ...
        if repay_type == "2":
            ...
            # repay_apply_data["repayGuaranteeFee"] = 8
            # repayGuaranteeFee = 8
            repay_apply_data["repayGuaranteeFee"] = math.floor(
        ...
        if repay_type == "7":
            repay_apply_data["repayPrincipal"] = round(float(asset_repay_plan['pre_repay_principal']), 2)  # 本金
            # repay_apply_data["repayAmount"] = 243.03
            # repay_apply_data["repayInterest"] = 0
            # repay_apply_data["repayGuaranteeFee"] = 0
            self.log.demsg('还款的日期为{}天'.format(days))
            if days == 0:
                repay_apply_data["repayInterest"] = 0  # 上期账单日提前结清，利息应收0
            # if days >= 30:
            #     repay_apply_data["repayInterest"] = repay_apply_data["repayInterest"]  # 计提超过30天，按照30天收整期利息
            ...
            channel_agency_repay_plan_detail_sync = self.MysqlBizImpl.get_op_channel_database_info(
                'channel_agency_repay_plan_detail_sync', loan_invoice_id=loanInvoiceId, repay_term=repayTerm)
            # repay_apply_data["repayGuaranteeFee"] = 1
            # repayGuaranteeFee = 1
            repay_apply_data["repayGuaranteeFee"] = math.floor(
                channel_agency_repay_plan_detail_sync['pre_guarantee_fee'] / 30 * days * 100) / 100
            repay_apply_data["repayAmount"] = round(
    ...
    # 担保费同步
    def syncGuaranteePlan(self, loanInvoiceId, flag=None, **kwargs):
        """
        注意：键名必须与接口原始数据的键名一致

        """
        strings = str(int(round(time.time() * 1000))) + str(random.randint(0, 9999))
        syncGuaranteePlan_data = dict()
        # head
        syncGuaranteePlan_data['requestSerialNo'] = 'requestNo' + strings + "_1400"
        syncGuaranteePlan_data['requestTime'] = self.date
        syncGuaranteePlan_data['merchantId'] = self.merchantId
        # body
        syncGuaranteePlan_data['loanInvoiceId'] = loanInvoiceId
        syncGuaranteePlan_data['flag'] = flag  # flag-同步标识，枚举值：loan-放款阶段、repay-还款阶段；

        # syncGuaranteePlan_data['guaranteePlans'][0]['period'] = period
        # syncGuaranteePlan_data['guaranteePlans'][0]['guaranteeAmt'] = guaranteeAmt

        # 更新 payload 字段值
        syncGuaranteePlan_data.update(kwargs)
        parser = DataUpdate(self.cfg['syncGuaranteePlan']['payload'], **syncGuaranteePlan_data)
        self.active_payload = parser.parser
    ...
    def payment(self, loan_invoice_id=None, repay_type='0', payment_type="5", repayTerm=None, **kwargs):
        ...
        url = API['request_host_api'].format(self.env) + self.cfg['payment']['interface']
        self.log.demsg('h5还款url为{}'.format(url))
