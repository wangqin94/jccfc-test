...
from src.test_data.module_data import ZMYC
from utils.Apollo import Apollo
from utils.FileHandle import Files


class ZMYCBizImpl(MysqlInit):
    def __init__(self, data=None, encrypt_flag=True, person=True):
        ...
        self.interestRate = getInterestRate(ProductIdEnum.ZMYC.value)
        # 初始化产品、商户
        self.productId = ProductIdEnum.ZMYC.value
        self.merchantId = EnumMerchantId.ZMYC.value
    ...
    def get_user_info(self, data=None, person=True):
        ...
        if data:
            ...
            if person:
                base_data = get_base_data(str(self.env) + ' -> ' + str(ProductEnum.ZMYC.value))
            else:
