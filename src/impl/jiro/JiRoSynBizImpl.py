...
class JiRoSynBizImpl(JiRoBizImpl):
    def __init__(self, data=None, person=False):
        super().__init__(data=data, person=person)

        self.CheckBizImpl = CheckBizImpl()
        self.weiCaiCheckBizImpl = JiRoCheckBizImpl(self.productId, self.data)
    ...
    def preLoanApply(self, bill_date=None, term=12):
        ...
        # 接口层校验授信结果是否符合预期
        self.weiCaiCheckBizImpl.jiro_check_credit_apply_status(thirdApplyId)
        ...
        # 接口层校验授信结果是否符合预期
        self.weiCaiCheckBizImpl.jiro_check_loan_apply_status(thirdApplyId)
...
if __name__ == "__main__":
    jiroBizImpl = JiRoSynBizImpl()
    jiroBizImpl.preLoanApply(bill_date="2023-05-24")
