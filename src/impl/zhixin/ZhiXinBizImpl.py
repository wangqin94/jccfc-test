...
class ZhiXinBizImpl(ZhiXinBiz):
    ...
    # 准备借款数据
    def preLoanapply(self, month=3, bill_date=None):
        """
        多层接口业务组装关键字：放款
        ...
        # 接口层校验授信结果是否符合预期
        status = self.ZhiXinCheckBizImpl.check_credit_apply_status(self.data, creditRes['userId'], creditRes['creditApplyNo'])
        ...
        # 接口层校验授信结果是否符合预期
        status = self.ZhiXinCheckBizImpl.check_loan_apply_status(self.data, loanRes['userId'], loanRes['loanApplyNo'])
        ...
        credit_loan_invoice = self.MysqlBizImpl.get_credit_database_info('credit_loan_invoice', loan_apply_id=loan_apply_info['loan_apply_id'])
        loan_date = str(loan_date).replace("-", '')
        self.MysqlBizImpl.update_asset_database_info('asset_loan_invoice_info', attr="loan_invoice_id='{}'".format(credit_loan_invoice['loan_invoice_id']), apply_loan_date=loan_date)
