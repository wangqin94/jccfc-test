...
def pre_loan_data(fqlBizImpl, fqlCreditCheckBizImpl, fqlLoanCheckBizImpl):
    ...
    with allure.step("数据库层校验支用结果是否符合预期"):
        ...
        repayPlanInfo = fqlLoanCheckBizImpl.queryRepayPlanInfo(loan_invoice_id=loanInvoice['loan_invoice_id'], current_num=1)
...
class TestCase(object):
    ...
    def test_billDate_repay1(self, fqlRepayCheckBizImpl, job, redis):
        ...
        with allure.step('分期乐下载文件并首次还款-任务流'):
           job.trigger_job_byJobId(job_group='5', job_type='MAIN_TRIGGER_JOB', id='545946619904192512', executeBizDate=account_date)
           time.sleep(5)
    ...
    def test_overdue_repay2(self,fqlRepayCheckBizImpl, job, redis):
        ...
        with allure.step("校验当前借据状态"):
            info = fqlRepayCheckBizImpl.queryAssetLoanInvoiceInfo('asset_loan_invoice_info', attr=None, loan_invoice_id=gl.get_value('loanInfo')['loan_no'])
