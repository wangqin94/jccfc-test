...
class TestCase(object):
    ...
    def test_billDate_repay1(self, fqlRepayCheckBizImpl, job):
        ...
        with allure.step("设置还款日期"):
            RepayPublicBizImpl().pre_repay_config(repayDate=repay_date_bill)

        log.info('按期还款日期---------------: {}'.format(repay_date_bill))
        with allure.step('分期乐还款-下载文件并入库'):
            job.trigger_job_byJobId(job_id='545946619904192512', executeBizDate=repay_date_bill.replace('-', ''))
            time.sleep(5)

        with allure.step('分期乐还款-轮询还款'):
            job.trigger_job_byJobId(job_id='359432679567003648', executeBizDate=repay_date_bill.replace('-', ''))
            time.sleep(5)
    ...
    def test_overdue_repay2(self, fqlRepayCheckBizImpl, job):
        ...
        with allure.step("设置还款日期"):
            RepayPublicBizImpl().pre_repay_config(repayDate=repay_date_ove)
        ...
        log.info('逾期还款日期---------------: {}'.format(repay_date_ove))
        with allure.step('分期乐还款-下载文件并入库'):
            job.trigger_job_byJobId(job_id='545946619904192512', executeBizDate=repay_date_ove.replace('-', ''))
            time.sleep(5)

        with allure.step('分期乐还款-轮询还款'):
            job.trigger_job_byJobId(job_id='359432679567003648', executeBizDate=repay_date_ove.replace('-', ''))
            time.sleep(5)
    ...
    def test_settle_repay3(self, fqlRepayCheckBizImpl, job):
        ...
        with allure.step('生成提前还款文件并上传gfs'):
