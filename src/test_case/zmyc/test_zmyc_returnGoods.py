...
class MyTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.zmyc = ZMYCBizImpl(data=data)
    ...
    def test_repay_apply(self, loanInvoiceId=None, repayDate=None, repayTerm=2):
        ...
        if not loanInvoiceId:
            credit_loan_invoice = self.zmyc.MysqlBizImpl.get_credit_database_info('credit_loan_invoice', certificate_no=data['cer_no'])
            loanInvoiceId = credit_loan_invoice['loan_invoice_id']
        self.zmyc.returnGoods_apply(loanInvoiceId=loanInvoiceId, term=repayTerm, repayDate=repayDate)
